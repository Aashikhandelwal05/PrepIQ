      ...
      <div className="space-y-4">
        {resourceErrorMessage && (
          <div className="rounded-xl border border-destructive/30 bg-destructive/10 p-3 text-sm text-destructive">
            Unable to refresh some workspace data.{" "}
            {resourceErrorMessage}
          </div>
        )}
...
function AppRoutes() {
  ...
  const { profile, saveProfile, profileError } =
    useCareerProfile(user?.id);

  const { sessions, addSession, sessionsError } =
    useInterviewSessions(user?.id);

  const { attempts, addAttempt, attemptsError } =
    useMockAttempts(user?.id);

  const { jobs, addJob, updateJob, jobsError } =
